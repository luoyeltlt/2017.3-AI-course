...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindow(wc.lpszClassName, widen(Window->winName),
		WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT:
		...
		if (_visualPage == 0) makeBitmap(hdc, Screen->buffer1->data, Screen->buffer1->sizeX, Screen->buffer1->sizeY, 24);
		else makeBitmap(hdc, Screen->buffer2->data, Screen->buffer2->sizeX, Screen->buffer2->sizeY, 24);
		EndPaint(hwnd, &ps);
		return 0;
...
void putChar(char ch, int x, int y) {
	...
	for (j = 0; j < 13; j++)
		for (k = 7; k >= 0; k--)
			if ((letters[ch][j] >> k) & 1)putPixel(x + 7 - k, y + 15 - j);
}
